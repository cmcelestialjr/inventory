                        ...
                        {deductions.map((deduction) => {
                            return (
                                <div
                                    ...
                                    className={`p-3 bg-white border rounded-lg shadow-sm text-sm text-gray-700 cursor-pointer
                                        ${selectedDeduction?.id === deduction.id ? 'bg-blue-100 border-blue-500' : 'hover:bg-blue-50'}`}
                                >
                                    <div className="font-medium">{deduction.name} {deduction.group || ""}</div>
                                    <div className="text-blue-600 font-semibold mt-1">
                                        Amount: {deduction.amount > 0 ? new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP' }).format(deduction.amount) : 0}
                                    </div>
                                </div>
                            );
                        })}
